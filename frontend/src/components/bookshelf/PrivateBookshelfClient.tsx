...
/**
 * PrivateBookshelfClient — 个人书架主客户端组件
 *
 * 与 BookshelfClient 结构一致，区别在于：
 * - 使用 useBooks('private') 和 useCategories('private')
 * - 包含 BookUploadButton 上传入口
 * - 需要登录，未登录时重定向到 /auth
 */
...
export function PrivateBookshelfClient() {
  ...
  }, [selectedFile, uploadTitle, uploadBook])
    ...
    return (
      <div className="flex items-center justify-center py-20">
        <motion.span
          ...
          className="inline-flex text-[var(--color-text-subtle)]"
        >
          <SpinnerGap size={28} />
        </motion.span>
      </div>
  ...
  return (
    ...
    >
      ...
      {/* Book edit dialog (user owns all private books) */}
      {editingBook && (
        <BookEditDialog
          book={editingBook}
          open={!!editingBook}
